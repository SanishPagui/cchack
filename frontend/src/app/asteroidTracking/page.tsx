...
import { Calendar, Search, AlertTriangle, Zap, Ruler, Clock, RefreshCw } from "lucide-react"
...
export default function AsteroidTracker() {
  ...
  const fetchAsteroids = async (date: Date, showRefreshing = false) => {
    if (showRefreshing) {
      setRefreshing(true)
    } else {
      setLoading(true)
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch asteroid data: ${response.status}`)
      }
  ...
  const handleRefresh = () => {
    fetchAsteroids(selectedDate, true)
  }
    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-black text-white">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Near Earth Object Monitoring</p>
                </div>
              </div>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="mb-6 p-4 bg-gray-900 rounded-lg border border-gray-800">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-white font-medium">
                {format(selectedDate, "MMM dd, yyyy")} - {format(addDays(selectedDate, 7), "MMM dd, yyyy")}
              </span>
            </div>
          </div>
          ...
          {!error && (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredAsteroids.map((asteroid) => (
                <Card key={asteroid.id} className="bg-gray-900 border-gray-800 hover:border-gray-700 transition-colors">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <CardDescription className="text-gray-400 mt-1">ID: {asteroid.id}</CardDescription>
                      </div>
                      {asteroid.is_potentially_hazardous_asteroid && (
                        <Badge variant="destructive" className="bg-red-900 text-red-100 border-red-800">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* Close Approach Data */}
                    {asteroid.close_approach_data.slice(0, 1).map((approach, index) => (
                      <div key={index} className="space-y-3">
                        <div className="flex items-center space-x-2">
                          <Clock className="w-4 h-4 text-gray-400" />
                          <span className="text-sm text-gray-400">Close Approach:</span>
                          <span className="text-white text-sm">
                            {format(new Date(approach.close_approach_date), "MMM dd, yyyy")}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p className="text-white font-medium">
                              {Number.parseInt(approach.relative_velocity.kilometers_per_hour).toLocaleString()} km/h
                            </p>
                          </div>
                          <div>
                            <p className="text-gray-400">Miss Distance</p>
                            <p className="text-white font-medium">
                              {Number.parseInt(approach.miss_distance.kilometers).toLocaleString()} km
                            </p>
                          </div>
                        </div>
                      </div>
          ...
          {/* No Results */}
          {!error && filteredAsteroids.length === 0 && searchTerm && (
            <div className="text-center py-12">
              <p className="text-gray-400">No asteroids found matching "{searchTerm}"</p>
            </div>
          )}
        </div>
